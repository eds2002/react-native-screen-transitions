...
import { handoffVideoPlayer } from "./handoff-video-player";
...
type HandoffMultiflowScreenProps = {
...
export function HandoffMultiflowScreen({
	backgroundColor,
	letter,
	next,
	placementStyle,
	showVideo = false,
}: HandoffMultiflowScreenProps) {
	...
	return (
		<View
			testID={`handoff-multiflow-${letter}`}
		...
		>
			<View style={[styles.placement, placementStyle]}>
				<Animated.View style={[styles.motionFrame, randomizedPlacementStyle]}>
					...
					<Transition.Boundary
						id={BOUNDARY_ID}
						testID={`handoff-multiflow-${letter}-boundary`}
						handoff
						style={[
							styles.frame,
			...
			<View style={styles.actions}>
				<Pressable
					testID={`handoff-multiflow-${letter}-small`}
					style={({ pressed }) => [
						styles.button,
				...
				<Pressable
					testID={`handoff-multiflow-${letter}-large`}
					style={({ pressed }) => [
						styles.button,
						styles.randomizeButton,
						pressed ? styles.buttonPressed : undefined,
				...
				{canGoBack ? (
					<Pressable
						testID={`handoff-multiflow-${letter}-back`}
						style={({ pressed }) => [
							styles.button,
							pressed ? styles.buttonPressed : undefined,
						]}
				...
				{next ? (
					<Pressable
						testID={`handoff-multiflow-${letter}-next`}
						style={({ pressed }) => [
							styles.button,
							pressed ? styles.buttonPressed : undefined,
						]}
						onPress={() =>
							router.push(
								buildStackPath(`bounds/handoff-multiflow/${next}`) as never,
							)
						}
					>
