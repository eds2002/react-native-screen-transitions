...
export const GESTURE_EXAMPLES: GestureExampleDefinition[] = [
	...
	},
	{
		...
		description: "Child screen toggles the parent route's vertical gesture.",
		subtitle: "navigation.getParent().setOptions({ gestureEnabled })",
		routeLabel: "ancestor gestureEnabled + vertical gesture",
		tint: "#40505E",
		notes: [
			"Disable the parent gesture, then drag down: the route should resist and reset.",
			"Enable the parent gesture, then drag down again: the nested route should dismiss.",
			"Horizontal drags should not dismiss because the parent route is vertical-only.",
		],
	},
];
...
export function GestureSuiteScreen({ id }: { id: GestureExampleId }) {
	...
	const suitePath = buildStackPath("gestures");
	const stackHome = buildStackPath();
