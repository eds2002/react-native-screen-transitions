...
summary: "Build custom screen transitions, snap sheets, overlays, and bounds-driven navigation motion."
description: "Build custom screen transitions, snap sheets, overlays, and bounds-driven navigation motion."
eyebrow: "Get Started"
group: "Get Started"
order: 1
to: /
...
`react-native-screen-transitions` is a transition toolkit for apps that need more control than the platform defaults.
...
Blank Stack is the default recommendation. It uses the same transition runtime with React Navigation and Expo Router through host-specific entry points.
...
## One set of transition APIs

Presets, interpolators, gestures, overlays, snap points, and bounds transitions work the same way across the supported navigation hosts. The integration pages only cover how each host creates and mounts Blank Stack.
