...
summary: "Write screen interpolators with per-screen state, derived helpers, runtime options, and slot output."
description: "Write screen interpolators with per-screen state, derived helpers, runtime options, and slot output."
...
## Related Transition Scopes

Use [`useScreenAnimation`](/api/use-screen-animation) inside a component to read another transition scope:
...
const style = useAnimatedStyle(() => ({
  opacity: interpolate(
    parentAnimation.get()?.progress ?? 1,
    [0, 1],
    [0.8, 1],
    "clamp",
  ),
...
Negative depths resolve ancestors and positive depths resolve descendants. You can also pass a screen's unique `transitionKey`, such as `useScreenAnimation("feed")`. Non-current targets can return `null`.

The interpolator's `transition()` accessor was removed in 4.1. Call the hook in a component under a transition scope, not inside `screenStyleInterpolator`. See the [4.1 migration note](/changelog/updating-to-4-1).
...
| `settled` | `1` when the screen is visually close enough to its animation target to be treated as settled |
...
| `initiator` | Gesture that activated the current attempt, or `"none"` |
| `handoff` | Release-time gesture snapshot for dismiss handoff animations; `handoff.active` identifies the released gesture |
...
Use this for values that must change per frame on the UI thread, such as dynamic `gestureSensitivity`. Use `navigation.setOptions()` for React-side option changes such as toggling `gestureEnabled` or switching `gestureDirection` while a screen stays mounted. Structural options such as `gestureTracking` must stay on the static screen options.
