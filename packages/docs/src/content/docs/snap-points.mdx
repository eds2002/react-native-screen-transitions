...
To measure a specific container, put its `styleId` directly in `snapPoints`:
...
The container's height determines the auto snap point, capped at the screen height. Its position is ignored. Use one matching container per screen and mount it during initial layout: opening at `"auto"` waits for that measurement. Subsequent layout changes update the auto detent using the existing auto-sizing behavior. Use `"auto"` instead of a custom style ID to measure the entire screen content. Use one measured entry (`"auto"` or a style ID) per list. The resolved height is sorted with the numeric snap points.
...
Set `sheetSnapBehavior` to `"step"` when every adjacent interval should receive
a full normalized drag range:
...
Each gesture can reach only the next higher or lower snap point. Release
velocity can decide whether that adjacent step commits, but cannot skip a
detent. The default `"continuous"` behavior maps movement directly to global
screen progress and may cross multiple detents in one gesture.
