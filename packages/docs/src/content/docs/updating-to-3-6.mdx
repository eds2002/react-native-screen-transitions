...
Reveal requires a source boundary, a matching destination boundary, and the static `navigationMaskEnabled` screen option. The helper exposes tuning options such as `borderRadius`, `borderContinuous`, `maxSensitivity`, `velocityDepth`, `gestureProgressMode`, `disablePointerEventsTillElementTransition`, and `maskSizingMode`.
