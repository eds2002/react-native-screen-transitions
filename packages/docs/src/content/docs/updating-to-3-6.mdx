...
Reveal requires a source boundary and a matching destination boundary. Current versions use built-in clipping; `navigationMaskEnabled` and `maskSizingMode` are deprecated and ignored. The helper exposes tuning options such as `borderRadius`, `borderContinuous`, `maxSensitivity`, `velocityDepth`, `disablePointerEventsTillElementTransition`, and `maskSizingMode`.
