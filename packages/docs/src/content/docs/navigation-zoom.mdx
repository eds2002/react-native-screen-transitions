...
`navigation.zoom()` returns `clip` geometry automatically on both platforms. No enable option or additional dependency is needed. To customize the visible region, override `clip` after spreading the helper result; set it to `undefined` to disable clipping.

`navigationMaskEnabled` is deprecated and ignored.
