...
The bounds runtime now avoids several old sources of repeated measurement work. Destination measurement is stabilized before the transition resolves its link, close-time scroll listeners are detached, and overscrolled scroll containers are normalized before their bounds are written.
