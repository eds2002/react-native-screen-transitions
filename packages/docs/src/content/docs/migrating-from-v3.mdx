...
When the transition needs its own gesture response, layer choreography, or visual treatment, build it in `screenStyleInterpolator` with `bounds(id).styles()` or `bounds(id).values()`. See [Bounds Transitions](/shared-elements) for the lower-level geometry helpers and [Zoom](/navigation-zoom) for the built-in recipe.
