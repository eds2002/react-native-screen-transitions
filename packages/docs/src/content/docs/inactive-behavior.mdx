...
<Note>
  `hide` uses `display: "none"`. Gesture reattachment after a hidden screen
  becomes active again has been verified with React Native Gesture Handler
  2.31.1. If a restored screen is visible but its gestures do not respond,
  update Gesture Handler to that known-good version.
</Note>
