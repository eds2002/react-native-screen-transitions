...
One known caveat is platform parity during closing transitions.

On iOS, `react-native-screens` backs stack presentation with UIKit's `UINavigationController`, which handles closing states more gracefully. In cases where the closing transition exposes content behind the outgoing screen, iOS can allow touches through to that content.

Android is built on `Fragment`s. During a closing transition, the outgoing screen can block touch events entirely, which makes the same flow feel less fluid than iOS.
