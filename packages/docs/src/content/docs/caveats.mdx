...
summary: "Runtime trade-offs and integration limits to test before shipping."
description: "Understand Blank Stack, native-stack adapter, bounds handoff, and platform trade-offs."
eyebrow: "Guides"
group: "Guides"
order: 1
to: /caveats
...
Blank Stack uses React 19.2 Activity to pause covered screens while preserving paint needed by an active transition. This removes most inactive React work, but it does not make retained component state free. Use `unmount` for routes that should release their subtree, and test stateful native views in every retention mode.
...
`Transition.Boundary` handoff moves one live payload between matching boundary hosts.

It's useful for video players, maps, and other stateful native content, but it also makes platform view lifecycle and compositor behavior observable.
