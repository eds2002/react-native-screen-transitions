...
6. move embedded flows to blank stack independent mode

If you are already on v3.4 or v3.5 and want the current release changes, read [Updating to 3.6](/updating-to-3-6) after this page.
...
Use blank stack for embedded or independent flows:
...
Blank stack covers that use case directly and is the supported path going forward.

For the v3 blank-stack model, including when to use `independent`, see [Independent Stacks](/independent-stacks).
...
`useScreenGesture()` returns a gesture for coordination with Gesture Handler. It does not configure `gestureEnabled`, `gestureDirection`, or other screen options.
...
- move embedded flows to blank stack independent mode
