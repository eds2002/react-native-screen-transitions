...
When no target is provided, zoom creates an aspect-ratio-preserving virtual destination. Narrow sources fill the screen height and attach to its leading edge; other sources fill the screen width and attach to its top edge. `target: "bound"` stays opt-in and waits for the matching destination measurement instead of temporarily falling back to fullscreen.
