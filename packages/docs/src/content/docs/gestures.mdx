...
| `gestureTracking` | `"auto" \| "never" \| "always"` | Controls whether the screen tracks live gesture values. Defaults to `"auto"`. |
...
This deprecates `gestureDrivesProgress` in favor of a more descriptive option name. The older `gestureDrivesProgress: false` form still maps to `"freeform"` for compatibility, but new code should use `gestureProgressMode`.
...
const grandparentGesture = useScreenGesture({ depth: -2 });

const parentAnimation = useScreenAnimation({ depth: -1 });
const childAnimation = useScreenAnimation({ depth: 1 });
```

`useScreenGesture()` returns a navigator pan gesture for Gesture Handler relations:

```tsx
const parentGesture = useScreenGesture({ depth: -1 });

const customPan = usePanGesture({
  requireToFail: parentGesture,
});
```
