...
export const useBuildRootGestures = ({
	...
	currentScreenKey,
	navigation,
}: BuildRootGesturesHookProps) => {
	const dimensions = useWindowDimensions();
	const currentScreen = ConfigStore.use(
		useCallback((state) => state.screens[currentScreenKey], [currentScreenKey]),
	);
	...
	const translateX = GestureStore.getMutable(currentScreenKey, "x");
	const translateY = GestureStore.getMutable(currentScreenKey, "y");
	const normalizedX = GestureStore.getMutable(currentScreenKey, "normalizedX");
	const normalizedY = GestureStore.getMutable(currentScreenKey, "normalizedY");
	const isDragging = GestureStore.getMutable(currentScreenKey, "isDragging");
	const isDismissing = GestureStore.getMutable(
		currentScreenKey,
		"isDismissing",
	);
	const progress = ScreenProgressStore.getScreenProgress(currentScreenKey);
