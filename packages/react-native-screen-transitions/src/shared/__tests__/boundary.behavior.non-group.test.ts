...
describe("Non-Group Flow", () => {
	...
	it("retains measured entry style metadata alongside bounds for matched tags", () => {
		const tag = makeTag("styled-card");
		const bounds = createBounds(5, 10, 80, 90);
		const styles = { opacity: 0.5, zIndex: 2 };

		BoundStore.entry.set(tag, "screen-a", {
			bounds,
			styles,
		});

		const measuredEntry = BoundStore.entry.getMeasured(tag, "screen-a");
