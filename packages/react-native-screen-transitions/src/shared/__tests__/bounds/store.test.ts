...
import { getPairKeyForDestination } from "../../stores/bounds/internals/links";
...
describe("BoundStore.link.getPair", () => {
	it("finds a parent pair from its nested grouped destination", () => {
		const pairKey = createScreenPairKey("source", "nested-route");
		...
		BoundStore.link.setDestination(
			...
			"violet",
			"destination-leaf",
			createBounds(100, 120),
			{},
			"colors",
		);
		BoundStore.link.setActiveGroupId(pairKey, "colors", "cyan");
		BoundStore.link.setSource(
			pairKey,
			"cyan",
			"source",
		...
		expect(
			getPairKeyForDestination("colors:cyan", "destination-leaf"),
		).toBe(pairKey);
	});
