...
>(
	...
	const handleCloseRoute = ({ route }: { route: BaseStackRoute }) => {
		...
		// If React Navigation still owns the route, ask it to pop. The next
		// props update will reconcile the removed route into our local snapshot.
		...
		// React Navigation already removed the route, so this is just local
		// cleanup for the descriptor we kept alive during the close animation.
	...
	const requestDismiss = ({ route }: { route: BaseStackRoute }): boolean => {
		...
		// Soft dismiss: mark the route as closing in the local snapshot without
		// mutating React Navigation. The actual pop happens when close finishes.
