...
/**
 * Resolves slot styles for the current screen pass and restores transition-owned
 * keys when a slot stops returning them.
 */
export const resolveSlotStyles = ({
	...
	const appendResolvedSlot = (slotId: string) => {
		...
		// Most frames do not need a reset patch. In that common case we can keep
		// the existing normalized slot object and skip rebuilding another wrapper.
