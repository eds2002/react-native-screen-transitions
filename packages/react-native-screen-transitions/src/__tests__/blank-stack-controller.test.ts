...
describe("createBlankStackController", () => {
	...
	for (const reuseOptions of [true, false]) {
		it(`keeps typed screen values with ${reuseOptions ? "identical" : "equivalent"} options`, () => {
			const navigation = createNavigation();
			const route = createRoute("filters");
			const options = { gestureEnabled: false };
			const renderedValues: string[] = [];
			const propsFor = (query: string) =>
				createProps(
					[route],
					{
						filters: {
							...createDescriptor(
								route,
								navigation,
								reuseOptions ? options : { ...options },
							),
							render: () => {
								renderedValues.push(query);
								return null;
							},
						},
					},
			...
			const controller = createBlankStackController(propsFor(""));

			for (const query of ["a", "ap", "apple", "apple ", "apple pie", ""]) {
				controller.update(propsFor(query));
				controller.getSnapshot().state.descriptors.filters?.render?.();
				expect(renderedValues.at(-1)).toBe(query);
			}
		});
	}

	it("reuses the snapshot when the renderer and options are unchanged", () => {
		const navigation = createNavigation();
		const route = createRoute("filters");
		const render = () => null;
		const propsFor = () =>
			createProps(
				...
				{
					filters: {
						...createDescriptor(route, navigation, { gestureEnabled: false }),
						render,
					},
				},
				navigation,
