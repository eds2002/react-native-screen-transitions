...
const scenes = Object.fromEntries(["builder-parent", "builder-child"].map((key) => [key, {
	route: { key, name: key },
	activity: "active",
	descriptor: {
		route: { key, name: key },
		options: { screenStyleInterpolator: () => ({ content: { opacity: 1 } }) },
		navigation: { getState: () => ({ routes: [{ key }] }) },
	},
}]));

mock.module("../../providers/stack/blank-stack.provider", () => ({
	useBlankStackStore: (selector: (value: { scenesByKey: typeof scenes }) => unknown) => selector({ scenesByKey: scenes, scenes: Object.values(scenes), focusedIndex: 1, navigatorKey: "builder-test-navigator" }),
}));

// Exercise Builder without mounting motion or slots.
const { BuilderProvider, useBuilderStore, useOptionalBuilderStore, getBuilderStore } = await import("../../providers/screen/builder/builder.provider");
...
function BlockParent() {
	...
	useLayoutEffect(() => {
		state.actions.requestLifecycleTransition(LifecycleTransitionRequestKind.Open, 1);
...
describe("BuilderProvider", () => {
	it("exposes the same readiness through context and key lookup before slots exist", () => {
		act(() => {
			renderer = create(<><RemoteProbe /><BuilderProvider routeKey="builder-child"><LocalProbe /></BuilderProvider></>);
		});

		const system = local.animationState;
		system.actions.requestLifecycleTransition(LifecycleTransitionRequestKind.Open, 1);
	...
	it("inherits a parent's visibility block while remaining ready itself", () => {
		act(() => {
			renderer = create(<BuilderProvider routeKey="builder-parent"><BlockParent /><BuilderProvider routeKey="builder-child"><LocalProbe /></BuilderProvider></BuilderProvider>);
		});
		...
		expect(wrappers[1].props.style[1]).toEqual({ transform: [{ translateY: 0 }] });
	});
	it("keeps values on rerender and releases the registered state on unmount", () => {
		const render = () => <BuilderProvider routeKey="builder-child"><LocalProbe /></BuilderProvider>;
	...
	it("preserves values across React Activity hiding and showing", () => {
		const render = (mode: "visible" | "hidden") => <Activity mode={mode}><BuilderProvider routeKey="builder-child"><LocalProbe /></BuilderProvider></Activity>;
		act(() => { renderer = create(render("visible")); });
		const state = local.animationState;
		state.targetProgress.set(0.6);
		state.measuredContentLayout.set({ width: 200, height: 300 });
		act(() => { renderer!.update(render("hidden")); });
		act(() => { renderer!.update(render("visible")); });
		expect(local.animationState).toBe(state);
		expect(getBuilderStore("builder-child").animationState).toBe(state);
		expect(state.targetProgress.get()).toBe(0.6);
		expect(state.measuredContentLayout.get()).toEqual({ width: 200, height: 300 });
	});

	it("snaps using the mounted builder's measured auto point and animation values", () => {
		act(() => { renderer = create(<BuilderProvider routeKey="builder-child"><LocalProbe /></BuilderProvider>); });
		const state = local.animationState;
		state.resolvedAutoSnapPoint.set(0.6);
		const originalRaf = globalThis.requestAnimationFrame;
		globalThis.requestAnimationFrame = (callback) => { callback(0); return 1; };
		try {
			expect(snapDescriptorToIndex({ ...local.descriptors.current, options: { snapPoints: [0.25, "auto", 1], transitionSpec: { expand: { duration: 0 } } } }, 1)).toBe(true);
			expect(state.targetProgress.get()).toBe(0.6);
	...
	it("registers nested relationships and removes them with Builder", () => {
		...
		expect(screenTopology.getRelationships("builder-child").parentScreenKey).toBe("builder-parent");
		expect(screenTopology.getRelationships("builder-parent").activeChildScreenKey).toBe("builder-child");
		expect(screenTopology.getRelationships("builder-parent").activeChildNavigatorKey).toBe("builder-test-navigator");
