...
import { ScreenKeysContext } from "../contexts/screen-keys";
...
type SceneViewProps = {
	...
	descriptor: AwareStackDescriptor;
	previousDescriptor?: AwareStackDescriptor;
	nextDescriptor?: AwareStackDescriptor;
			...
			<NavigationContext.Provider value={navigation}>
				<NavigationRouteContext.Provider value={route}>
					<ScreenStackItem
						...
						gestureResponseDistance={gestureResponseDistance as any}
					...
					>
						<ScreenKeysContext.Provider
							value={{
								...
								previousScreenKey: previousDescriptor?.route.key,
...
type Props = {
	...
	navigation: AwareNativeStackActionHelpers;
...
export function AwareNativeStackView({
	...
	const onWillDisappear = useStableCallback(() => {
	...
	const onWillAppear = useStableCallback(() => {
		(navigation as any).emit?.({
			type: "transitionStart",
			data: { closing: false },
			target: route.key,
	...
	const onAppear = useStableCallback(() => {
		(navigation as any).emit?.({
			type: "transitionEnd",
			data: { closing: false },
			target: route.key,
	...
	const onDisappear = useStableCallback(() => {
		(navigation as any).emit?.({
			type: "transitionEnd",
			data: { closing: true },
			target: route.key,
	...
	const onDismissed = useStableCallback((event: any) => {
	...
	const onHeaderBackButtonClicked = useStableCallback(() => {
		(navigation as any).dispatch?.({
			...StackActions.pop(),
			source: route.key,
			target: state.key,
	...
	const onNativeDismissCancelled = useStableCallback((event: any) => {
		(navigation as any).dispatch?.({
			...StackActions.pop(event.nativeEvent.dismissCount),
			source: route.key,
			target: state.key,
	...
	const onGestureCancel = useStableCallback(() => {
		(navigation as any).emit?.({
			type: "gestureCancel",
			target: route.key,
		});
	...
	const onSheetDetentChanged = useStableCallback((event: any) => {
		(navigation as any).emit?.({
